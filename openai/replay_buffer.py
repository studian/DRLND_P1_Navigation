...
from openai.segment_tree import SumSegmentTree, MinSegmentTree
import torch
...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        obses_t = torch.from_numpy(np.array(obses_t)).float().to(self.device)
        actions = torch.from_numpy(np.array(actions)).long().to(self.device)
        rewards = torch.from_numpy(np.array(rewards)).float().to(self.device)
        obses_tp1 = torch.from_numpy(np.array(obses_tp1)).float().to(self.device)
